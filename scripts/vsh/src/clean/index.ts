...
import { colors, getPackages, rimraf, spinner } from '@dag/node-utils'
...
async function runClean({ delLock = false, dirs = [], recursive }: CleanCommandOptions) {
    ...
    spinner(
        {
            successText: colors.green(`clean uo all ${cleanDirsText} success.`),
...
export function defineCleanCommand(cac: CAC) {
    cac.command('clean [dirs...]')
        ...
        .action(async (dirs, { delLock, recursive }) => {
