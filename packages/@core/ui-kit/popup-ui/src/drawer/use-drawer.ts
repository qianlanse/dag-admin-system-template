import type { DrawerApiOptions, DrawerProps, ExtendedDrawerApi } from './types'
...
import DagDrawer from './drawer.vue'

const USER_DRAWER_INJECT_KEY = Symbol('DAG_DRAWER_INJECT')
...
export function useDagDrawer<TParentDrawerProps extends DrawerProps = DrawerProps>(
    ...
    mergedOptions.onOpenChange = (isOpen: boolean) => {
        ...
        injectData.option?.onOpenChange?.(isOpen)
    }
    ...
    const Drawer = defineComponent(
        (props: DrawerProps, { attrs, slots }) => {
            return () => h(DagDrawer, { ...props, ...attrs, drawerApi: extendedApi }, slots)
        },
        // eslint-disable-next-line vue/one-component-per-file
        {
            name: 'DagDrawer',
