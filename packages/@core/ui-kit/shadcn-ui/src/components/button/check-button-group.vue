<script setup lang="ts">
    ...
    import type { DagButtonGroupProps, ValueType } from './button'

    import { computed, ref, watch } from 'vue'

    import { Circle, CircleCheckBig, LoaderCircle } from '@dag-core/icons'
    import { DagRenderContent } from '@dag-core/shadcn-ui'
    import { cn, isFunction } from '@dag-core/shared/utils'

    import { objectOmit } from '@vueuse/core'

    import DagButtonGroup from './button-group.vue'
    import Button from './button.vue'

    defineOptions({
        name: 'DagCheckButtonGroup'
    })

    const props = withDefaults(defineProps<DagButtonGroupProps>(), {
        gap: 0,
        multiple: false,
        showIcon: true,
        size: 'middle'
...
<template>
    <DagButtonGroup :size="props.size" :gap="props.gap" class="dag-check-button-group">
        <Button
