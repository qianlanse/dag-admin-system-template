<script setup lang="ts">
    ...
    import { ArrowLeft, RotateCw } from '@dag/icons'
    ...
    const showBack = computed(() => props.status === '403' || props.status === '404')

    const showRefresh = computed(() => props.status === '500' || props.status === 'offline')
...
<template>
    <div class="flex size-full flex-col items-center justify-center duration-300">
        ...
        <component v-else-if="fallbackIcon" :is="fallbackIcon" class="h-1/3 w-1/2 lg:w-1/4" />
        <div class="flex-col-center">
            ...
            <DagButton v-else-if="showBack" size="lg" @click="handleBack">
                <ArrowLeft class="mr-2 size-4" />
                {{ $t('common.backToHome') }}
            </DagButton>
            <DagButton v-else-if="showRefresh" size="lg" @click="handleRefresh">
