<script setup lang="ts">
    ...
    import { LockKeyhole } from '@dag/icons'
    import { $t, useI18n } from '@dag/locales'
    import { storeToRefs, useAccessStore } from '@dag/stores'

    import { useScrollLock } from '@dag-core/composables'
    import { useDagForm, z } from '@dag-core/form-ui'
    import { DagAvatar, DagButton } from '@dag-core/shadcn-ui'
    ...
    const [Form, { form, validate }] = useDagForm(
        reactive({
            commonConfig: {
                hideLabel: true,
    ...
    /** 切换 */
    function handleToggleUnlockForm() {
    ...
    <div class="bg-background fixed z-[2000] size-full">
        ...
        <transition name="slide-right">
            ...
            >
                <div class="flex-col-center mb-10 w-[90%] max-w-[300px] px-4">
                    <DagAvatar :src="avatar" class="enter-x mb-6 size-20" />
                    <div class="enter-x mb-2 w-full items-center">
                        <Form />
                    </div>
                    <DagButton class="enter-x w-full" @click="handleSubmit">
        ...
        >
            <div v-if="showUnlockForm" class="enter-x mb-2 text-2xl md:text-3xl">
                {{ hour }}:{{ minute }}
                <span class="text-base md:text-lg">{{ merdiem }}</span>
            </div>
